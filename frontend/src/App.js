...
function App() {
  ...
  const combineRules = async () => {
    try {
      ...
      // If there's already a combined AST, combine it with the last rule
      const currentCombinedAst = combinedAst || rules[0];
      const response = await axios.post('http://localhost:5000/combine_rules', {
        rules: [currentCombinedAst, rules[rules.length - 1]],
        ...
        <div className="form-container">
          <div className="form-group">
            <input
              ...
              placeholder="Rule ie: age > 30"
              className="form-input"
            />
            <button onClick={createRule} className="form-button">Create Rule</button>
          </div>
          <div className="form-group">
            <select value={selectedOperator} onChange={handleOperatorChange} className="operator-select">
